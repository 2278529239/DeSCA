...
def load_test_best_model(args):
    ...
    #     # 增量训练，每年初始化使用上一年的模型
    #     prev_year_path = osp.join(args.model_path, args.logname+"-"+str(args.seed), str(args.year-1))
    #     if not osp.exists(prev_year_path):
    #         raise FileNotFoundError(f"No model folder found for previous year {args.year-1}: {prev_year_path}")
        
    #     loss = [f[:-4] for f in os.listdir(prev_year_path)]
    #     loss = sorted(loss)
    #     load_path = osp.join(prev_year_path, loss[0]+".pkl")  # 取最小 loss 的模型
    loss = []
