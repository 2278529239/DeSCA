...
def train(inputs, args):
    ...
    if args.init == True and args.year > args.begin_year:
        ...
        if args.method in ['EAC', 'EACpre', 'DCRNNpre', 'DCRNNplus', 'PDFormerpre', 'PDFormerplus']:
        ...
        if args.method == 'EAC'or args.method == 'EACpre':
        ...
        if args.method == 'DCRNNpre'or args.method == 'DCRNNplus':
            args.logger.info("[CL] Before Freeze backbone parameters, trainable parameters:")
            for n, p in model.named_parameters():
                if p.requires_grad:
                    args.logger.info(f"    [Trainable] {n}")
        ...
        if args.method == 'PDFormerpre' or args.method == 'PDFormerplus':
            args.logger.info("[CL] Before Freeze backbone parameters, trainable parameters:")
            for n, p in model.named_parameters():
                if p.requires_grad:
                    args.logger.info(f"    [Trainable] {n}")
        ...
        if args.method in ['EAC', 'EACpre', 'STID', 'gwnet', 'itransformer','DCRNNpre','DCRNNplus', 'PDFormerpre', 'PDFormerplus']:
        ...
        model = gnn_model
        if args.method in ['EAC', 'EACpre','DCRNNpre','DCRNNplus']:
            model.expand_adaptive_params(args.graph_size)
